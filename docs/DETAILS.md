...
> ### Corrections, 2026-09-21 — read [HANDOFF.md](docs/HANDOFF.md) first
>
> Three changes below invalidate parts of this document. The rung ladder itself is unaffected
...
>    on this page is at n≥10 unless it says otherwise.
> 2. **Residual learning is gone, and it invalidated every earlier fusion number.** Nets were
>    trained on `ddG − forest_prediction` with the forest added back at inference, so a net
>    with zero skill still scored ~0.50. Standing alone the same architecture scores 0.308.
> 3. **The conclusion above is strengthened, not weakened.** With the forest no longer
>    propping it up, the network loses on both splits and by a wider margin on the looser one.
>    The random forest remains the model.
...
interaction ([`BACKBONE_COMPARISON.md`](docs/BACKBONE_COMPARISON.md)).
...
[`ARCHITECTURE.md`](docs/ARCHITECTURE.md) carries the full specification: the pipeline from a SKEMPI
...
- [`ARCHITECTURE.md`](docs/ARCHITECTURE.md) — encoder pipeline, parameter budget, and what 750
  training rows can support
- [`PLAN.md`](docs/PLAN.md) — time budget, model ladder, bottleneck map, deferred ideas with triggers
- [`EDA_FINDINGS.md`](docs/EDA_FINDINGS.md) — what the data audit found and what it changed
- [`PRIOR_WORK.md`](docs/PRIOR_WORK.md) — literature calibration: what "good" looks like here
